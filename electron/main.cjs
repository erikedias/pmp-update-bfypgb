...
function buildReportAnalysisPrompt(d) {
  ...
  const canc = d.mode === "cancelamento";
  return [
    canc
      ? `Você é analista de mídia paga. Escreva um PANORAMA GERAL da plataforma ${d.label} para o cliente "${d.clientName}", cobrindo TODO o período de ${d.monthLabel} (relatório de encerramento/cancelamento). Olhe o desempenho consolidado do período inteiro, não mês a mês. Este texto vai DIRETO para o cliente — já é a análise final.`
